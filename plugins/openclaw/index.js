...
function determineTier(type, content) {
    // Hot tier for critical items
    if (type === "error" || type === "secret" || type === "finance")
        return "hot";
...
function extractEntities(text, source = "telegram") {
    const entries = [];
    const seen = new Set();
    const addEntry = (content, type, context, tags) => {
        // Skip duplicates
    ...
    urls?.forEach(u => {
        if (!u.includes("github.com"))
            addEntry(u, "url", "Reference URL", ["link", "reference"]);
    });
    ...
    for (const [, config] of Object.entries(CATEGORY_KEYWORDS)) {
        if (config.keywords.some(k => lowerText.includes(k))) {
            ...
            addEntry(text.substring(0, 200), config.type, `Auto-detected ${config.type} from conversation`, [config.type, "auto-detected"]);
            break;
        }
    }
    return entries;
...
function saveToBrainX(entry, logger) {
    ...
    register(api) {
        api.registerHook("message_received", async (ctx) => {
            ...
            entries.forEach(entry => {
                if (saveToBrainX(entry, api.logger))
                    saved++;
            });
            if (saved > 0) {
